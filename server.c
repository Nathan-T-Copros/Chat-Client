...
int main( int argc, char **argv )
{
	...
	printf("\n\n*Chat Started*\n"); //print message when socket user connect to the server
	
	//receive messages
	while( (length = recv( clientSocket, buffer, sizeof(buffer) - 1, 0)) > 0 && strcmp( buffer, "quit" ) != 0 ) //quit when user inputs "quit"
	{		
		buffer[length] = '\0';	//format buffer to a string by terminating it with a '\0'	
		printf("%s\n", buffer);
	}	
